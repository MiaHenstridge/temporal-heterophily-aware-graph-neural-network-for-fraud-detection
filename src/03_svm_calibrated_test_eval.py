...
if __name__ == "__main__":
    ...
    if FEAT_AUGMENT:
        ...
        with open('hyperparameter_tuning/svm_best_params.yaml', 'r') as f:
            best_params = yaml.safe_load(f)
    else:
        # load best params for no feat augment
        with open('hyperparameter_tuning/svm_nofeat_best_params.yaml', 'r') as f:
            best_params = yaml.safe_load(f)

    
    # convert string to correct data types (C, loss, class_weight)
    best_params = {
        'C': float(best_params['C']),
        'loss': best_params['loss'],
        'class_weight': eval(best_params['class_weight']),
        'feat_augment': bool(best_params['feat_augment']),
    }
    ...
    with mlflow.start_run():
        ...
        # create model
        logger.info(f"Best Hyperparameters: {best_params}, seed: {RANDOM_SEED}")
