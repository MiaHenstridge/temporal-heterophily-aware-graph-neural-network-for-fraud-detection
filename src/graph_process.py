...
class NeighborFinder:
    ...
    def find_before(self, src_idx, cut_time):
        ...
        # binary search to find the rightmost neighbor with ts < cut_time
        while left + 1 <= right:
            mid = (left + right) // 2
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbors=20):
        """
        Takes a **batch** of (node, cut_time) pairs and returns fixed-size neighbor arrays `[batch_size, num_neighbors]`. 
        Two sampling modes:

        - **`uniform=True`**: Randomly samples `num_neighbors` from all valid past neighbors. 
        - **`uniform=False`** (default): Takes the **most recent** `num_neighbors` neighbors (the last `num_neighbors` before cut_time). 
            Results are right-aligned in the output array — earlier positions are zero-padded.

        The fixed output shape is important because it feeds directly into the attention layers as a regular tensor.
        ...
        return out_ngh_node_batch, out_ngh_t_batch, out_ngh_eidx_batch
